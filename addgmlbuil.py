...
def CityGML2IFC(path: str, filename: str):
    ...
    print("Creating baulinie annotations...")
    for key, data in baulinie_dict.items():
        points = data["points"]
        ifc_points = []
        for x, y, z in points:
            ifc_point = ifcfile.create_entity(
                "IfcCartesianPoint",
                Coordinates=[x, y, z]
            )
            ifc_points.append(ifc_point)
        polyline = ifcfile.create_entity("IfcPolyline", Points=ifc_points)
        representation = ifcfile.create_entity(
        ...
        annotation = ifcfile.create_entity(
            "IfcAnnotation",
            GlobalId=new_guid(),
            Name=f"Baulinie_{key}",
            ObjectType="BAULINIE",
            ObjectPlacement=root_site_placement,
            Representation=ifcfile.create_entity("IfcProductDefinitionShape", Representations=[representation])
        )
        ifcfile.create_entity(
            "IfcRelContainedInSpatialStructure",
            GlobalId=new_guid(),
            RelatingStructure=site_main,
            RelatedElements=[annotation]
        )
    print("Creating baugrenze annotations...")
    for key, data in baugrenze_dict.items():
        points = data["points"]
        ifc_points = []
        ...
        annotation = ifcfile.create_entity(
            "IfcAnnotation",
            GlobalId=new_guid(),
            Name=f"Baugrenze_{key}",
            ObjectType="BAUGRENZE",
            ObjectPlacement=root_site_placement,
            Representation=ifcfile.create_entity("IfcProductDefinitionShape", Representations=[representation])
        )
        ifcfile.create_entity(
    ...
    print("Creating flurstueck geometry for IfcSite...")
    all_flurstueck_points = []
    for key, data in flurstueck_dict.items():
        points = data["points"]
        all_flurstueck_points.extend(points)

    if all_flurstueck_points:
        ifc_points = []
        for x, y, z in all_flurstueck_points:
            ifc_point = ifcfile.create_entity(
                "IfcCartesianPoint",
                Coordinates=[x, y, z]
            )
        ...
        site_shape = ifcfile.create_entity(
            "IfcProductDefinitionShape",
            Representations=[representation]
        )
        site_flurstueck.Representation = site_shape
